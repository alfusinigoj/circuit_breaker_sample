...
1. dotnet restore --configfile nuget.config
1. Publish app to a directory selecting the framework and runtime you want to run on. (e.g. `dotnet publish -f netcoreapp2.2 --self-contained`)
1. Push the app using the appropriate manifest. (e.g. `cf push`)
...
To see the logs as you startup and use the app: `cf logs fortuneui-aj`
